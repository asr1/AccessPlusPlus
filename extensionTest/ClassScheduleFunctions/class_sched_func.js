...
function remRepeats(arr){ 
   ...
   for (i = 0; i < result.length; i++){
    ...
    if (!/[a-zA-Z]/.test(result[0]) || /%20/g.test(result[0])) { //splice(0,0) does not work. we need an extra check
...
function parseName(nome){
    if (debug) toPrint += "\nIn parseName nome is: " + nome;
	var splited = nome.split(',');
    if (debug) toPrint += "\nIn parseName splitted is: " + splited;
	splited[0] + ',';
    if (debug) toPrint += "\nIn parseName splitted[0] is: " + splited[0];
	return splited;
}
...
$(document).ready(function() {
 ...
 if (url == accessPlus || url == accessPlus1){
  ...
  for (i = 0; i < updProfs.length; i++){ 
   ...
   if (!(i%2 == 0)) {
    if(debug) toPrint += "\nodd->nome[0]  "+nome[0];
    if(debug) toPrint += "\nodd->updProfs["+i+"] "+updProfs[i];
    $(box).append(cssEntry('#E8E8E8', updProfs[i], nome[0]));
   }
   
   else {
    if(debug) toPrint += "\neven->nome[0]  "+nome[0];
    if(debug) toPrint += "\neven->updProfs["+i+"] "+updProfs[i];
