...
function CreateSchedule(start, end,  eventStart,  eventEnd,  WeekDays)
{
    while(start <= end)
	{
       //alert(start);           
	   ...
	   var eventStart = new Date(start.setHours(12,39));
	   var eventEnd = new Date(start.setHours(14,0));
	 ...
	 var eventEndString = (eventEnd.getMonth()+1).toString().concat("/").concat(eventEnd.getDate().toString()).concat("/").concat(eventEnd.getFullYear().toString()).concat(" ").concat(eventEnd.getHours().toString()).concat(":").concat(eventEnd.getMinutes().toString());//.concat(" PM"));
	   ...
	   if(eventStart.getDay() === 2 || eventStart.getDay() === 4)
	   {	   
			cal.addEvent('Com Sci 311', 'Algorithms', 'Atanassoff 310',new Date(eventStartString) ,new Date(eventEndString));
	   }   
	}
...
function init() { 
	...
	var StartTime =  new Date(new Date(2015,07,24).setHours(12,39));
	var EndTime = new Date(new Date(2015,07,24).setHours(14,0));
	var WeekDays=[2,4];//Tuesday and Thursday
	CreateSchedule(StartDate, EndDate, StartTime, EndTime , WeekDays);
